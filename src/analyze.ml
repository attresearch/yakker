...
let prec_rewrite_complex gr =
  ...
  *)
  let add_prec_attrs tokmap = function
    ...
    | LexerDef _ | LexerDecl _ | LexerDecl2 _ | SingleLexerDecl _ -> () in
...
(* OLD VERSION: i think it carries too much , and the wrong, information for tokens. YHM. *)
...
(*       (\** tokenizer, (ocaml_constructor,carried_type) list: store all tokens from the same tokenizer *\) *)
  ...
  type tokens = string * string list
      (** tokenizer, token_name list: group all tokens from the same tokenizer *)
  ...
  let non_singleton_tok tokenizer tokname =
    { nonempty = Cs.empty (), [(tokenizer, [tokname])];
      epsilon = false;
      maybe_empty = [];
      maybe_nonempty = []}
  ...
  let to_rule = function
    | {nonempty = cs,ts;
       ...
       maybe_empty = [];
       maybe_nonempty = []} ->
        let r_cs_opt = if Cs.is_empty cs then None else Some (Minus.cs2rule cs) in
        let r_ts_opt = match ts with
          | [] -> None
          | ts ->
              let toks = List.flatten (List.map snd ts) in
              Some (Gul.mkALT (List.map (fun s -> mkSYMB(s, None, None)) toks)) in
        (match r_cs_opt, r_ts_opt with
           | None, None -> if eps then mkLIT "" else invalid_arg "first set cannot be converted to a rule"
           | None, Some r -> if eps then mkOPT r else r
           | Some r, None -> if eps then mkOPT r else r
           | Some r1, Some r2 -> let r = mkALT2 (r1, r2) in if eps then mkOPT r else r)
    | _ -> invalid_arg "first set cannot be converted to a rule"

(* function to get the string of token list field if t.nonempty *)
let pr_ts ts =
  let printone (tokenizer, tokens) =
    Printf.sprintf "%s: %s" tokenizer (String.concat "," tokens) in
        ...
        | RuleDef(n, r, a) ->
            ...
            let (fs1, has_change) =
              match search_tokmap tokmap n with
                ...
                | Some (tokenizer, _, _) ->
                    (non_singleton_tok tokenizer n, true)
            in
            let newmap =
              if has_change then
                if fv then
...
let first r first_map =
  let rec loop r =
    match r.r with
      | Symb (n1, _, _, _) ->  (* TODO: attributes *)
          (try PMap.find n1 first_map with Not_found -> Util.warn Util.Sys_warn (Printf.sprintf "Unable to compute %s's FIRST set." n1);
             empty ())
      | When _
