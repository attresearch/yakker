  ...
  *)
  let fuse = function
    ...
    | actions_rev ->
        Gil.Action {P.e = fun () ->
                      ...
                      P.lam2 (fun p v ->
                             List.fold_left (fun e f -> P.app2 f (P.Var p) e) (P.Var v) actions)} in
  ...
  let fuse_box actions b n = match actions with
    ...
    | actions_rev ->
        ...
        let box =
          {P.e = (fun () ->
                    let actions_hoas = List.rev_map (fun a -> P.InjectE a) actions_rev in
                    P.lam3 (fun v p ykb ->
...
let fuse_gil_definitions2 ds = List.map (fun (n, r) ->
                                          (n, Gil.Meta.phoas_simplify_to_string $ fuse_gil_rhs2 $ remove_empty_lit $| r)) ds

let fuse_gil2 gr = gr.Gul.gildefs <- fuse_gil_definitions2 gr.Gul.gildefs
