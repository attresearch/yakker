  ...
  let rec loop r = begin
    ...
    | Seq(r1,early,late,r2) ->
        ...
        if need_early_extent || need_late_extent then begin
          let extent before after = Printf.sprintf "Yakker.get_substring %s %s" before after in
          let extent_l before after = Printf.sprintf "Yakker.get_string %s %s ykinput" before after in
