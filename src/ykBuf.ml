...
(** [fill2 ykb n] ensures that at least [n] bytes of buffered data
    ...
    @return whether n bytes are available. If they are not available,
...
(** If f is an ocamllex entry point, then (wrap_ocamllex f) is
    ...
    Note that the parsing engine handles save and restore of ykBuf
    state, so that boxes (including those generated by
    [wrap_ocamllex]) don't have to.
*)
